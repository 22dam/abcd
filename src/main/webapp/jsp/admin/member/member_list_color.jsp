		...
		<header>
		    ...
		    <nav>
		    	<ul>
			    	...
			    	<li><a href="${contextPath}/admin/product/orderList">주문 리스트</a></li>
					...
					<c:forEach var="member" items="${members}" varStatus="st">
						...
						<div style="width:3400px;height:70px;">
							...
							<input type="hidden" id="count_${member.id}" value="${st.index}" />
					   		...
					   		<!-- checkbox -->
							<div style="float:left;width:100px;height:70px;background:cyan;">
							...
							<div style="float:left;width:80px;height:70px;background:orange;">
								${pageVO.limit * (pageVO.currPage-1) + st.count}
							</div>
						
							<div style="float:left;width:100px;height:70px;background:orange;">
								${member.id}
							</div>
							
							<div style="float:left;width:100px;height:70px;background:lightgray;">
								**********
							</div>
							
							<!-- 이메일 -->
							<div style="float:left;width:400px;height:70px;background:pink;">
							...
							<div style="float:left;width:100px;height:70px;background:pink;">
							...
							<!-- 연락처 -->
							<div style="float:left;width:400px;height:70px;background:green;">
								
								<%-- ${member.phone} --%>
								<input type="text" onclick="alert('click phone')" class="form-control"
					    		       id="phone_${member.id}" value="${member.phone}" />
								
							</div>
							
							<div style="float:left;width:100px;height:70px;background:pink;">
								<a href="#" data-toggle="tooltip" id="phone_update_btn_${member.id}" onclick="alert('연락처 클릭')" class="btn" title="이메일 수정">
			    					<span class="material-icons">
										check_circle_outline
									</span>
								</a>
							...
							<!-- 우편번호 -->
							<div style="float:left;width:120px;height:70px;background:violet;">
								<%-- ${member.zipNum} --%>
								<%-- <input type="text" id="zipNum_${member.id}" class="form-control"
									   onclick="alert('zipNum_${member.id}')" value="${member.zipNum}" readonly /> --%>
								<div id="zipNum_${member.id}" class="p-2 bg-info"><%-- onclick="getPostcodeAddress('${member.id}')" --%>
							...
							<!-- 기본주소 -->
							<div style="float:left;width:400px;height:70px;background:Beige;">
								<div id="address1_${member.id}" class="p-2 bg-white">
							...
							<!-- 상세주소 -->
							<div style="float:left;width:400px;height:70px;background:AliceBlue;">
								<div id="address2_${member.id}" class="p-2 bg-white" contenteditable="false"
							...
							<div style="float:left;width:200px;height:70px;background:AntiqueWhite;">
								${member.useyn == 'y' ? "회원" : "탈퇴"}
							</div>
							
							<div style="float:left;width:350px;height:70px;background:Chartreuse;">
								<fmt:formatDate value="${member.indate}" pattern="yyyy년  M월 d일 HH:mm:ss"/>
							</div>

							<div style="float:left;width:200px;height:70px;background:CadetBlue;">
							...
							<div style="float:left;width:100px;height:70px;background:Cornsilk;" class="ml-2">
