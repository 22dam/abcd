...
@Service
@Slf4j
public class CartService { // 서비스에는 dao, TransactionTemplate를 와이어링한다.
	...
	public boolean checkCart(List<CartVO> cartList, CartVO cartVO) {
		...
		for (CartVO defaultCartVO : cartList) { // 카트리스트에 상품 담는 동안
			
			if (defaultCartVO.equals(cartVO)) { // 기존 상품과 새로 담는 상품이 동일하다면
				...
				log.info("CartVO.getQuantity() : " + cartVO.getQuantity()); // 새로 담은 수량
				
				// defaultCartVO 안에 set적재량(기존 적재량 + 새로담은 적재량)
	...
	public List<CartVO> listCart(String id) {
		...
		return txTemplate.execute(txStatus-> { 
			return cartDAO.listCart(id); // 카트로 아이디 태워서 보내기
		});
	}
	...
	public boolean updateCart(CartVO cartVO) {
		...
		return txTemplate.execute(status -> {
			...
			try {
				...
				if (cartDAO.updateCart(cartVO) == true) { // 카트 수정 성공시, flag true!
					log.info("카트 수정 성공");
					flag = true;
				} else {
					log.info("카트 수정 실패");
					throw new Exception("카트 수정 실패"); // 실패시, catch로
				}
	 ...
	 * @return 카트 삭제 성공 여부
	 */
	@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class) // 롤백처리하면 try-catch 생략가능
	public boolean deleteCart(int cseq) {
		...
		return txTemplate.execute(status -> {
			...
			if (cartDAO.deleteCart(cseq) == true) { // 카트 삭제 성공시, flag true!
				log.info("카트 삭제 성공");
				flag = true;
			} else {
				log.error("카트 삭제 실패");
